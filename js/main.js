//TODO: get working on touchscreens.
    //maybe use mobile detection to switch from onmouse to mouse-over?
// 1: implement scroll gesture
  // implement $P recognizer?
// 2: implement other gestures? (project requirement)
// 2.5 previous, next, undo/delete, select
// 3: list traversal
// 4: implement actions? (optional, play music, etc) (mara, Android intents?)
...
$(function() {
  ...
  function setFullScreen(bool) {
    ...
    switch (isFullScreen){
      case true:
        ...
        sourceCanvas.canvas.width  = w;
        sourceCanvas.canvas.height = h;
        break;
      case false:
        $('#results').show();
        $('#stroke-images').show();
  ...
  //init
  //hackish -> would like to have train and switch user screens
  //? would give help
  //train, user would be commands in queue

  setFullScreen(window.mobile);
  ...
  function onGesture(result) {
    ...
    if(isFullScreen){
      ...
      /*
       * 3 Display template match in <img id="matching-template">
       */
      ...
      $("#matching-template").attr("src", "images/templates/" + filename + ".png");
        ...
        /*
         * 4 Display score in <div id="match">
         */
      $('#match').html("<p> Detected: " + result.Name + " <br /> Confidence: " + Math.round(result.Score*100) + "% </p>");
    }

    /*
     * Finally clear <canvas id=gesture-input">
     */
    sourceCanvas.getContext('2d').clearRect( 0 , 0 , w , h );
  }
});